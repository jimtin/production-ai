...
\* `security-threat-model` is referenced throughout but not included in this repo (vendored Apache-2.0 work pending attribution — see the [README](../README.md#what-is-deliberately-not-here)). Bring your own threat-modeling skill; the call sites tell you exactly when it fires.
...
- **Specialist gates** own one dimension — design risk, frontend quality, validation readiness, coverage, removal safety, observability — and define what "covered" means for it.
...
5. **Escalation is part of the contract.** Cheap skills name the expensive skill to escalate to (`laptop` audits escalate repo upgrades to repo-specific work with `$test-readiness-preflight`), so scope creep gets routed instead of absorbed.
